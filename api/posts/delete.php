...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($id <= 0) {
        sendJsonResponse('error', 'Post ID is required.');
    }
    ...
    if ($post['user_id'] !== $user_id) {
        sendJsonResponse('error', 'Unauthorized to delete this post.');
    }
    $check_stmt->close();
    ...
    if ($stmt->execute()) {
        sendJsonResponse('success', 'Post deleted successfully.');
    } else {
        sendJsonResponse('error', 'Failed to delete post.');
    }
    $stmt->close();
} else {
